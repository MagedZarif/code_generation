...
void PrintTreeToFile(TreeNode* node, ofstream& outFile, int sh = 0)
{
    int i, NSH = 3;
    for (i = 0; i < sh; i++) outFile << " ";

    outFile << "[" << NodeKindStr[node->node_kind] << "]";

    if (node->node_kind == OPER_NODE) outFile << "[" << TokenTypeStr[node->oper] << "]";
    else if (node->node_kind == NUM_NODE) outFile << "[" << node->num << "]";
    else if (node->node_kind == ID_NODE || node->node_kind == READ_NODE || node->node_kind == ASSIGN_NODE)
        outFile << "[" << node->id << "]";

    if (node->expr_data_type != VOID) outFile << "[" << ExprDataTypeStr[node->expr_data_type] << "]";

    outFile << "\n";

    for (i = 0; i < MAX_CHILDREN; i++)
        if (node->child[i]) PrintTreeToFile(node->child[i], outFile, sh + NSH);
    if (node->sibling) PrintTreeToFile(node->sibling, outFile, sh);
}
...
struct SymbolTable
{
    ...
    void Print(ofstream& outFile)
    {
        ...
        for (i = 0; i < SYMBOL_HASH_SIZE; i++)
        {
            ...
            while (curv)
            {
                outFile << "[Var=" << curv->name << "][Mem=" << curv->memloc << "]";
                LineLocation* curl = curv->head_line;
                while (curl)
                {
                    outFile << "[Line=" << curl->line_num << "]";
                    curl = curl->next;
                }
                outFile << "\n";
                curv = curv->next_var;
...
void symbolTable(SymbolTable* symbol_table, TreeNode* node)
{
    if (node->node_kind == ID_NODE||node->node_kind == ASSIGN_NODE)
    {
    ...
    for(int i=0;i<MAX_CHILDREN;i++){
        if(node->child[i])
        {
            symbolTable(symbol_table,node->child[i]);
        }
    }
    if(node->sibling)
    {
        symbolTable(symbol_table, node->sibling);
    }
}

void syntaxTree (TreeNode* node)
{
   ...
   //check children
    for(int i=0;i<3;i++)
    {
        if(node->child[i])
        {
            syntaxTree( node->child[i]);
        }
    }


    //check if types except for the LessThan and Equal operators (integer)
    if(  node->node_kind==ID_NODE ||node->node_kind == NUM_NODE)
    {
        node->expr_data_type=INTEGER;
    }
//check if types except was operator and was (<) or (=) (boolean)
...
int Expressions(TreeNode* node, SymbolTable* symbol_table, int* arr)
{
    if (node->node_kind == ID_NODE)
    {
        return arr[symbol_table->Find(node->id)->memloc];
    }
    ...
    int x = Expressions(node->child[0], symbol_table, arr);
    int y = Expressions(node->child[1], symbol_table, arr);
...
void statement(TreeNode* node, SymbolTable* symbol_table, int* variables) {
    if (node->node_kind == READ_NODE)
    {
        cout << "Enter " << node->id << ": ";
        int val;
        cin >> val;
        variables[symbol_table->Find(node->id)->memloc] = val;
    }

    if (node->node_kind == WRITE_NODE) {
        int num = Expressions(node->child[0], symbol_table, variables);
        std::cout << "Val: " << num << endl;
    }


    if (node->sibling) {
        statement(node->sibling, symbol_table, variables);
    }
}

void simulation(TreeNode* node, SymbolTable* symbol_table, int* variables) {
    ...
    switch (node->node_kind) {
        ...
        case IF_NODE:
            if (Expressions(node->child[0], symbol_table, variables))
            {
                simulation(node->child[1], symbol_table, variables);
            }
            else if (node->child[2])
            {
                simulation(node->child[2], symbol_table, variables);
            }
            break;

        case REPEAT_NODE:
            simulation(node->child[0], symbol_table, variables);
            while (!Expressions(node->child[1], symbol_table, variables))
            {
                simulation(node->child[0], symbol_table, variables);
            }
            break;
        ...
        case ASSIGN_NODE:
            variables[symbol_table->Find(node->id)->memloc] =
                    Expressions(node->child[0], symbol_table, variables);
            break;

        default:
            statement(node, symbol_table, variables);
            break;
    }

    simulation(node->sibling, symbol_table, variables);
}
...
int main() {
    ...
    // Parse the input file and build the syntax tree
    TreeNode* parse_tree = Parse(&compilerInfo);
    ...
    symbolTable(&symbol_table, parse_tree);



    // Output the syntax tree and symbol table  to a file
    ofstream outFile("output.txt");
    if (outFile.is_open())
    {
        outFile << "The symbol table:\n";
        symbol_table.Print(outFile);
        outFile << "\n";
        outFile << "Parse Tree:\n";
        PrintTreeToFile(parse_tree, outFile);
        outFile << "\nParsing successful.";
        outFile.close();
    }
    ...
    simulation(parse_tree, &symbol_table, variables);
